...
class OutlinedEditBase:
    ...
    def paintEvent(self, e):
        ...
        if not self.label or not self.labelLabel.isUp:
            return painter.drawRoundedRect(self.rect().adjusted(pw, pt+pw, -pw, 0), r, r)
        ...
        # right line
        path.lineTo(w - 1, h - r - pw + 1)

        # bottom right arc
        path.arcTo(w - d - 1, h - d - pw + 1, d, d, 0, -90)

        # bottom line
        path.lineTo(r + pw, h - pw + 1)

        # bottom left arc
        path.arcTo(pw, h - d - pw + 1, d, d, -90, -90)
